...
public
  ...
  PackagingDataCalculator {
    ...
    // START calculate(IThrowableProxy-IThrowableProxy)//public void calculate(IThrowableProxy tp) {
    while (tp != null) {
...
// END calculate(IThrowableProxy-IThrowableProxy)//  }
	

    // START populateFrames(StackTraceElementProxy[]-StackTraceElementProxy[])//void populateFrames(StackTraceElementProxy[] stepArray) {
...
// END populateFrames(StackTraceElementProxy[]-StackTraceElementProxy[])//  }
	

    // START populateUncommonFrames(int-int-StackTraceElementProxy[]-StackTraceElementProxy[]-ClassLoader-ClassLoader)//void populateUncommonFrames(int commonFrames,
...
// END populateUncommonFrames(int-int-StackTraceElementProxy[]-StackTraceElementProxy[]-ClassLoader-ClassLoader)//  }
	

    // START calculateByExactType(Class-Class)//private ClassPackagingData calculateByExactType(Class type) {
    ...
    return cpd;
// END calculateByExactType(Class-Class)//  }
	

    // START computeBySTEP(StackTraceElementProxy-StackTraceElementProxy-ClassLoader-ClassLoader)//private ClassPackagingData computeBySTEP(StackTraceElementProxy step,
    ...
    cache.put(className, cpd);
    return cpd;
// END computeBySTEP(StackTraceElementProxy-StackTraceElementProxy-ClassLoader-ClassLoader)//  }
	

    // START getImplementationVersion(Class-Class)//String getImplementationVersion(Class type) {
    if (type == null) {
      return "na";
    }
...
// END getImplementationVersion(Class-Class)//  }
	

    // START getCodeLocation(Class-Class)//String getCodeLocation(Class type) {
    try {
      if (type != null) {
    ...
    return "na";
// END getCodeLocation(Class-Class)//  }
	

    // START getCodeLocation(String-String-char-char)//private String getCodeLocation(String locationStr, char separator) {
    ...
    return null;
// END getCodeLocation(String-String-char-char)//  }
	

    // START isFolder(int-int-String-String)//private boolean isFolder(int idx, String text) {
    return (idx != -1 && idx + 1 == text.length());
// END isFolder(int-int-String-String)//  }
	

    // START loadClass(ClassLoader-ClassLoader-String-String)//private Class loadClass(ClassLoader cl, String className) {
    if (cl == null) {
      return null;
    }
    try {
...
// END loadClass(ClassLoader-ClassLoader-String-String)//  }
     ...
     * @return
     */
    // START bestEffortLoadClass(ClassLoader-ClassLoader-String-String)//private Class bestEffortLoadClass(ClassLoader lastGuaranteedClassLoader,
                                    String className) {
...
// END bestEffortLoadClass(ClassLoader-ClassLoader-String-String)//  }
