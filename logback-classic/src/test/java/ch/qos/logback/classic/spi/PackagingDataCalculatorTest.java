...
public
  ...
  PackagingDataCalculatorTest {
    ...
    // START verify(ThrowableProxy-ThrowableProxy)//public void verify(ThrowableProxy tp) {
    for (StackTraceElementProxy step : tp.getStackTraceElementProxyArray()) {
      if (step != null) {
        assertNotNull(step.getClassPackagingData());
      }
    }
// END verify(ThrowableProxy-ThrowableProxy)//  }
...
// END smoke({FormalParametersInternal})//  }
    ...
    // START doCalculateClassPackagingData(boolean-boolean)//public void doCalculateClassPackagingData(
...
// END doCalculateClassPackagingData(boolean-boolean)//  }
	

    // START loop(int-int-boolean-boolean)//double loop(int len, boolean withClassPackagingCalculation) {
    long start = System.nanoTime();
    for (int i = 0; i < len; i++) {
      doCalculateClassPackagingData(withClassPackagingCalculation);
    }
    return (1.0 * System.nanoTime() - start) / len / 1000;
// END loop(int-int-boolean-boolean)//  }
	

    // START perfTest({FormalParametersInternal})//@Test
...
// END perfTest({FormalParametersInternal})//  }
	

    // START makeBogusClassLoader({FormalParametersInternal})//private ClassLoader makeBogusClassLoader() throws MalformedURLException {
    ClassLoader currentClassLoader = this.getClass().getClassLoader();
    return new BogusClassLoader(new URL[] {},
        currentClassLoader);
// END makeBogusClassLoader({FormalParametersInternal})//  }
	

    // START noClassDefFoundError_LBCLASSIC_125Test({FormalParametersInternal})//@Test
...
// END noClassDefFoundError_LBCLASSIC_125Test({FormalParametersInternal})//  }
